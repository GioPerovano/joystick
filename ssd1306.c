...
void ssd1306_line(ssd1306_t *ssd, uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1, bool value) {
    ...
    while (true) {
        ssd1306_pixel(ssd, x0, y0, value); // Draw current pixel
...
void ssd1306_draw_char(ssd1306_t *ssd, char c, uint8_t x, uint8_t y) {
    ...
    if (c >= '0' && c <= '9') {
        index = (c - '0' + 1) * 8; // Numbers from '0' to '9'
    } else if (c >= 'a' && c <= 'z') {
        index = (c - 'a' + 11) * 8; // Lowercase letters
    } else if (c >= 'A' && c <= 'Z') {
        index = (c - 'A' + 37) * 8; // Uppercase letters
    } else if (c >= '!' && c <= '/') {
        index = (c - '!' + 63) * 8; // Special characters
    } else {
        index = 0; // Unsupported characters
    }
