...
// Definição dos pinos e interfaces
...
uint16_t valor_x_anterior = 0, valor_y_anterior = 0; // Variáveis para armazenar as posições anteriores
...
// Função para imprimir apenas quando houver alteração no estado
void imprimir_estado_leds() {
    static bool estado_anterior_rgb = true;
    static bool estado_anterior_led_verde = false;

    if (estado_led_rgb != estado_anterior_rgb) {
        printf("LEDs RGB = %d\n", estado_led_rgb);
        estado_anterior_rgb = estado_led_rgb;
    }

    if (estado_led_verde != estado_anterior_led_verde) {
        printf("LED Verde = %d\n", estado_led_verde);
        estado_anterior_led_verde = estado_led_verde;
    }
}

// Função para imprimir a posição do joystick apenas quando houver alteração
void imprimir_posicao_joystick() {
    if (valor_x != valor_x_anterior || valor_y != valor_y_anterior) {
        printf("Posição do Joystick -> x = %d, y = %d\n", valor_x, valor_y);
        valor_x_anterior = valor_x;
        valor_y_anterior = valor_y;
    }
}
...
void tratar_interrupcao_botao(uint pino, uint32_t eventos) {
    ...
    if (tempo_atual - ultimo_tempo > 200000) {
        ...
        imprimir_estado_leds();  // Imprimir apenas quando houver mudança
    }
}
...
int main() {
    ...
    while (true) {
        ...
        imprimir_posicao_joystick();  // Imprimir apenas quando houver mudança na posição do joystick
        atualizar_display();
